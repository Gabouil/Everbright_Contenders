...
class Game():
                    ...
                    if self.player_turn.carte_oblige == False:
                        ...
                        if self.game_mots.button_mot2_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot2 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot3_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot3 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot4_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot4 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot5_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot5 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot6_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot6 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot7_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot7 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot8_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot8 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot9_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot9 = random.randint(0, 73)
                        ...
                        if self.game_mots.button_mot10_rect.collidepoint(pygame.mouse.get_pos()):
                            ...
                            if self.player_turn.mot_free:
                                ...
                                self.update_phrase()
                                self.player_turn.mot_free = False
                                self.game_mots.choix_mot10 = random.randint(0, 73)
    ...
    class Liste_mot():
        def __init__(self,game):
            self.game = game
            #Groupe nominaux
            self.votre_apparence = votre_apparence(self.game,"votre apparence", "GN")
            self.votre_prose = votre_prose(self.game, "votre prose", "GN")
            self.rien_que_vous_ecouter_jouter = rien_que_vous_ecouter_jouter(self.game, "rien que vous écouter jouter","GN")
            self.voyou_a_la_petite_semaine = voyou_a_la_petite_semaine(self.game, "voyou à la petite semaine", "GN")
            self.votre_talent = votre_talent(self.game, "votre talent", "GN")
            self.certains = certains(self.game, "certains", "GN")
            self.une_lice = une_lice(self.game, "une lice", "GN")
            self.affronter_une_gamine = affronter_une_gamine(self.game, "affronter une gamine", "GN")
            self.ton_style = ton_style(self.game, "ton style", "GN")
            self.pour_toi = pour_toi(self.game, "pour toi", "GN")
            self.ton_manque_dexperience = ton_manque_dexperience(self.game, "ton manque d'expérience", "GN")
            self.perroquet_frippe = perroquet_frippe(self.game, "perroquet frippe", "GN")
            self.la_tension = la_tension(self.game, "la tension", "GN")
            self.oh_petite_precieuse = oh_petite_precieuse(self.game, "oh petite precieuse", "GN")
            #Groupes Nominaux / complements
            self.petit_servant = petit_servant(self.game, "petit servant", "GNC")
            self.faquin = faquin(self.game, "faquin", "GNC")
            self.hypocrite = hypocrite(self.game, "hypocrite", "GNC")
            self.fripouille = fripouille(self.game, "fripouille", "GNC")
            self.paltoquet = paltoquet(self.game, "paltoquet", "GNC")
            self.prostitue_en_mailles = prostitue_en_mailles(self.game, "prostitué en mailles", "GNC")
            self.diable = diable(self.game, "diable", "GNC")
            self.chez_tes_parents = chez_tes_parents(self.game, "chez tes parents", "GNC")
            self.votre_odeur = votre_odeur(self.game, "votre odeur", "GNC")
            self.petit_minable = petit_minable(self.game, "petit minable", "GNC")
            self.quel_rate = quel_rate(self.game, "quel raté", "GNC")
            self.les_bottes_de_ton_maitre = les_bottes_de_ton_maitre(self.game, "les bottes de ton maître", "GNC")
            self.ta_place = ta_place(self.game, "ta place", "GNC")
            self.ton_visage = ton_visage(self.game, "ton visage", "GNC")
            self.pimbeche = pimbeche(self.game, "pimbêche", "GNC")
            self.cuistre = cuistre(self.game, "cuistre", "GNC")
            self.maraud = maraud(self.game, "maraud", "GNC")
            self.gourgandine = gourgandine(self.game, "gourgandine", "GNC")
            self.eunuque = eunuque(self.game, "eunuque", "GNC")
            self.pourceau = pourceau(self.game, "pourceau", "GNC")
            self.jeune_ignorant = jeune_ignorant(self.game, "jeune ignorant", "GNC")
            self.vieille_peau = vieille_peau(self.game, "vieille peau", "GNC")
            #Compléments
            self.retournez_dans_votre_bouge = retournez_dans_votre_bouge(self.game, "retournez dans votre bouge", "C")
            self.laisse_moi_rire = laisse_moi_rire(self.game, "laisse moi rire", "C")
            self.allons_donc = allons_donc(self.game, "allons donc", "C")
            self.de_vos_magouilles = de_vos_magouilles(self.game, "de vos magouilles", "C")
            self.vu_de_pres = vu_de_pres(self.game, "vu de près", "C")
            self.apparemment = apparemment(self.game, "apparemment", "C")
            self.ce_matin = ce_matin(self.game, "ce matin", "C")
            self.cest_ca_le_theme = cest_ca_le_theme(self.game, "c'est ça le thème", "C")
            self.cest_ca_que_taimes = cest_ca_que_taimes(self.game, "c'est ça que t'aimes", "C")
            self.ben_voyons = ben_voyons(self.game, "ben voyons", "C")
            self.si_on_ta_promis_la_gloire = si_on_ta_promis_la_gloire(self.game, "si on t'a promis la gloire", "C")
            self.pour_sur = pour_sur(self.game, "pour sûr", "C")
            #Groupes verbaux / compléments
            self.est_dans_un_musee = est_dans_un_musee(self.game, "est dans un musée", "GVC")
            self.on_ta_menti = on_ta_menti(self.game, "on t'a menti", "GVC")
            self.nest_pas_tres_joli_a_voir = nest_pas_tres_joli_a_voir(self.game, "n'est pas très joli à voir", "GVC")
            self.ce_que_tu_cherches = ce_que_tu_cherches(self.game, "ce que tu cherches", "GVC")
            self.sale_vaurien = sale_vaurien(self.game, "sale vaurien", "GVC")
            #Groupes verbaux
            self.est_si_famelique = est_si_famelique(self.game, "est si famelique", "GV")
            self.est_minable = est_minable(self.game, "est minable", "GV")
            self.vous_etes_vous_relu = vous_etes_vous_relu(self.game, "vous êtes-vous relu", "GV")
            self.parlez_nous = parlez_nous(self.game, "parlez-nous", "GV")
            self.rentre = rentre(self.game, "rentre", "GV")
            self.as_lair_si_bete = as_lair_si_bete(self.game, "as l'air si bête", "GV")
            self.na_rien_a_envier = na_rien_a_envier(self.game, "n'a rien à envier", "GV")
            self.est_tombe_dans_le_crottin = est_tombe_dans_le_crottin(self.game, "est tombe dans le crottin", "GV")
            self.est_palpable = est_palpable(self.game, "est palpable", "GV")
            self.tes_courgeux = tes_courgeux(self.game, "t'es courageux", "GV")
            self.a_plus_de_rides_que_toi = a_plus_de_rides_que_toi(self.game, "a plus de rides que toi", "GV")
            self.va_ten_lecher = va_ten_lecher(self.game, "va t'en lécher", "GV")
            self.on_se_demande_bien = on_se_demande_bien(self.game, "on se demande bien", "GV")
            self.tu_sors_de_ton_ecrin = tu_sors_de_ton_ecrin(self.game, "tu sors de ton ècrin", "GV")
            #Conjoction de coordination
            self.mais = mais(self.game, "mais", "Cc")
            self.ou = ou(self.game, "ou", "Cc")
            self.et = et(self.game, "et", "Cc")
            self.donc = donc(self.game, "donc", "Cc")
            self.or_ = or_(self.game, "or", "Cc")
            self.ni = ni(self.game, "ni", "Cc")
            self.car = car(self.game, "car", "Cc")
